...
import { ArrowRight, CircleDollarSign, ShieldCheck, Sparkles, ThumbsUp, Trophy } from "lucide-react";
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden bg-[var(--bg-main)] text-[var(--text-main)]">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-6xl flex-col px-6 pb-12 pt-6 md:px-10 lg:px-16 lg:pt-8">
        <header className="mb-10 flex items-center justify-between rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-card)]/70 px-4 py-3 backdrop-blur-md">
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--primary)] text-sm font-black text-white">⚡</div>
            <span className="text-sm font-bold tracking-wide text-[var(--text-main)] md:text-base">ROASTWAGER</span>
          </div>

          <div className="hidden items-center gap-2 text-xs md:flex">
            <span className="rounded-full border border-[var(--border-soft)] bg-[var(--bg-main)]/80 px-3 py-1 text-[var(--text-muted)]">Monad Testnet</span>
            <span className="rounded-full border border-[var(--border-soft)] bg-[var(--bg-main)]/80 px-3 py-1 text-[var(--text-muted)]">USDC Market</span>
          </div>
        </header>
        ...
        <section className="mb-10 flex flex-col items-center text-center">
          ...
          <p className="max-w-3xl text-sm leading-relaxed text-[var(--text-muted)] sm:text-base md:text-lg">
            RoastWager turns your hottest opinions into live, stake-backed prediction markets. Post, vote,
            resolve, and climb the leaderboard with every meaningful call.
          </p>

          <div className="mt-6 flex w-full max-w-md flex-col gap-3 sm:flex-row">
            <button className="group flex-1 rounded-2xl bg-[var(--primary)] px-5 py-3 text-sm font-bold text-white transition hover:bg-[var(--primary-soft)] sm:text-base">
              Launch App
              <ArrowRight size={16} className="ml-2 inline-block transition group-hover:translate-x-1" />
            </button>
            <button className="flex-1 rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-card)]/70 px-5 py-3 text-sm font-bold text-[var(--text-main)] backdrop-blur-md transition hover:border-[var(--primary-soft)] sm:text-base">
              Read Docs
            </button>
          </div>
        </section>
        ...
        <section className="mb-10 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-card)]/70 p-4 backdrop-blur-md">
            ...
            <p className="mt-1 text-lg font-black">Blind Pool + On-chain Resolve</p>
          </div>
          <div className="rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-card)]/70 p-4 backdrop-blur-md">
        ...
        <section className="mt-2 w-full">
          <h2 className="mb-4 text-center text-xl font-black md:text-2xl">How It Works</h2>

          <div className="mx-auto w-full max-w-5xl overflow-visible px-2 md:px-8">
            <Swiper
              effect="coverflow"
              centeredSlides
              slidesPerView={1.12}
              ...
              speed={900}
              autoplay={{ delay: 2600, disableOnInteraction: false, pauseOnMouseEnter: false, waitForTransition: false }}
                  ...
                  <SwiperSlide key={item.title} className="py-1">
                    <article className="rounded-3xl border border-[var(--border-soft)] bg-[var(--bg-card)]/82 p-8 text-center shadow-[0_24px_80px_rgba(0,0,0,0.4)] backdrop-blur-md md:p-10">
                      <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--primary)]/20">
                        <Icon size={32} className="text-[var(--primary-soft)]" />
                      </div>

                      <h3 className="mb-3 text-2xl font-black leading-tight">{item.title}</h3>
                      <p className="mx-auto max-w-sm text-sm leading-relaxed text-[var(--text-muted)] md:text-base">{item.desc}</p>
                    </article>
                  </SwiperSlide>
                );
              })}
